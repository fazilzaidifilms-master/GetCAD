...
- `migrations/` — schema, applied in filename order:
  ...
  - `0025_email_outbox.sql` — **transactional email as an OUTBOX.** Sending inline would mean a provider outage either 500s a submission (application lost over a mail hiccup) or drops the email silently. Instead the business function writes an `email_outbox` row in the SAME transaction as the work it acknowledges (`enqueue_email`, best-effort like `app.notify` — it can never roll back the submission it acknowledges), and a dispatcher drains it: `claim_emails` (SKIP LOCKED, reclaims rows stuck SENDING past a threshold) → send → `record_email_result` (idempotent, service_role only). Idempotency key is deterministic per event, so at-least-once delivery is not at-least-twice. `submit_marketing_lead` and `submit_designer_application` gain one enqueue call each (bodies carried forward verbatim). ANONYMITY: every email in this slice is to a person about their OWN action — no counterparty — and the payload carries only their first name; the rendered body lives in `core/email`, which is handed only those fields.
